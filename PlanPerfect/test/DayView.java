...
public class DayView {
    public static void main(String[] args) {
        ...
        try (Connection con = DriverManager.getConnection(url, user, password)) {
            String[] queries = { "SELECT * FROM 2023_Holidays;", "SELECT * FROM 2024_Holidays;",
                    "SELECT * FROM 2025_Holidays;" };
        ...
        SettingsButton.addActionListener(e -> {
            ...
            int Choosedchoice = JOptionPane.showOptionDialog(frm, "", "Settings", JOptionPane.YES_NO_OPTION,
            ...
            if (Choosedchoice == 1) {
                ...
                String[] fontTypes = { "Arial", "Times New Roman", "Helvetica", "Courier New", "Verdana",
                        "Lucida Console", "Tahoma", "Georgia" };

                String Type = (String) JOptionPane.showInputDialog(frm, "Select Font Type", "Font Type",
                        JOptionPane.PLAIN_MESSAGE, null, fontTypes, fontTypes[0]);
                if (Type != null) {
                ...
                while (true) {
                    ...
                    if (Display == JOptionPane.OK_OPTION) {
                        try {
                            ...
                            if (endDateInvalid) {
                                // Show a message dialog when the end date is not provided
                            ...
                            if (!Event_conflict) {
                                // Set the goal and goal progress frequency for each new event
                                for (CalendarEvent newEvent : newEvents) {
                                    String goal = goalField.getText();
                                    newEvent.setGoal(goal); // Set the goal for each new event
                                }

                                // Add new events to the 'events' list and update goal progress frequency
                                for (CalendarEvent newEvent : newEvents) {
                                    events.add(newEvent);
                                    int index = events.indexOf(newEvent) + 1;
                                    String goalProgressFrequency = index + "/" + newEvents.size();
                                    newEvent.setGoalProgressFrequency(goalProgressFrequency);
                                }
                                cal.repaint();
                                break;
                            } else {
